...
platform_do_upgrade() {
	...
	iyunlink,m87x2|\
	M87X2)
		get_image "$1" | dd of=/dev/mmcblk0 bs=4096
		;;
	*)
		nand_do_upgrade "$1"
		;;
...
platform_check_image() {
	...
	iyunlink,m87x2|\
	M87X2)
		magic="$(dd if="$1" bs=4 skip=110 count=1 2>/dev/null)"
		[ "$magic" != "OWRT" ] && {
			echo "Invalid image type."
			return 1
		}
...
platform_copy_config_emmc_for_m87x2() {
	mkdir -p /recovery
	mount -o rw,noatime /dev/mmcblk0p2 /recovery
